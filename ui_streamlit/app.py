...
import pandas as pd
from ui_streamlit.run import ejecutar_app_streamlit
...
def main():
    ...
    if not st.session_state["topbar_hidden"]:
        with st.container():
            ...
            with c3:
                st.write("")
                if st.button("Acceder", width="stretch"):
                    _try_access()
    ...
    if acceso_ok:
        ejecutar_app_streamlit(archivo, asignatura, curso, icono, selection_id=st.session_state[MATERIA_KEY])
    else:
